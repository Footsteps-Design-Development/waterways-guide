...
ini_set('error_log', '/home/customer/www/barges.org/public_html/components/com_waterways_guide/cron/classified_error.log'); // Replace '/path/to/error.log' with the actual path and filename where you want to store the error log.
	...
	if($num_rows>0){
		...
		foreach($mymembers as $row) {
			...
			if($membermessage){
				...
				$message.="".$emailfooter;
				//echo(nl2br("$memberemail<br>$message<br><br>"));
				if($livemail==1){
					if($memberemail){
						if($mailOn) {
