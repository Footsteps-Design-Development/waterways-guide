...
function load(tp) {
  if (GBrowserIsCompatible()) {
	...
	if(tp=="plot"){
		var thislat=parseFloat(document.getElementById("lat").innerHTML);
		var thislng=parseFloat(document.getElementById("lng").innerHTML);
		var center = new google.maps.LatLng(thislat,thislng);
	
	}else{
		var center = new google.maps.LatLng(<?php if($lat){echo($lat);}else{echo("48.89364");}?>,<?php if($lng){echo($lng);}else{echo("2.33739");}?>);
...
document.getElementById("lat").innerHTML = pt.lat().toFixed(5);
   document.getElementById("lng").innerHTML = pt.lng().toFixed(5);
   document.getElementById("latlng").value = pt.lat().toFixed(5) + "," + pt.lng().toFixed(5) ;
	});
...
function returnposition() {
	...
	window.opener.document.getElementById("GuideLat").value=lat;
	window.opener.document.getElementById("GuideLong").value=lng;
	//alert(lat+","+lng);
	window.close(self);
	//alert("choose");
}
