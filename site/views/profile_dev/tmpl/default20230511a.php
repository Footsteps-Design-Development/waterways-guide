				...
				$.ajax({
					...
					success: function(result){
					  ...
					  if(result){
						...
						span.textContent = "&nbsp;Coupon '"+coupon+"' validated - discount applied";
						el_down.innerHTML = "&nbsp;Coupon '"+coupon+"' validated - discount applied";
     
						changememtypeform();
					  }else{
						   
					   	alert("Error: Failed to validate the coupon");
					}
				});
			}else{
...
?>				default:
	...
	<tr><td class=profile_label><?php echo($servicetitle); ?></td><td class=profile_field valign="top"><?php echo($boxes); ?><div id="discount_message"></div></td></tr>			
		...
		if($ID=="new"){
			...
			//echo("<tr><td class=profile_label>Referrer email</td><td class=profile_field><span id='couponvalidate'><input id='coupon' name='coupon' type='text' size='30' maxlength='50'><span class='btn btn-info' onclick='validateCoupon()'>Validate</span></span></td></tr>");
...
<?php
	...
	if($membershipadmin==true && !$subaction){
		...
		echo("<tr><td colspan=2>Coupon status</td></tr>\n");
		if($num_coupons>0){
			...
			foreach($result as $resultrow) {
				...
				if(!$coupon_recipient_date_used){
					...
					$coupon_recipient_date_used="Not used";
				}
			...
			echo("<td class=profile_label valign='top'></td><td class=profile_field valign='top'>Non issued</td></tr>\n");
		}
