...
if (in_array("25", $userGroups)) {
	...
	if($userid==11838 || $userid==11557 || $userid==9065 || $userid==9246 || $userid==9026){

		//allow second member profile update

		//echo("userid=".$userid." Groups=".$userGroups);
		...
		echo("The member profile can only be updated by the main subscribing member and your membership is as a second family member. You can however add some personal details in the forum at <a href=\"index.php?option=com_kunena&func=profile&do=edit&Itemid=722\">here</a>.");
	...
	if ($subaction=="subscribe2" || $subaction=="Update" || $subaction=="Approve") {
		...
		if($subaction!="back"){
			...
			if($ID=="new"){
				...
				//Title FirstName	LastName	Title2	Initials2	FirstName2	LastName2	MembershipNo MemStatus	BasicSub	PaymentMethod	DatePaid		MemTypeCode	Situation	Address1	Address2	Address3	Address4	PostCode	Country	CountryCode	Telephone1	Telephone2	Email	NotesAdmin	PostZone	Level	LastUpdate	Services	Keywords	Groups	

				/*add to user groups

				14	2	25	26	Member single EU (old 1)

				15	2	19	20	Member family EU

				16	2	27	28	Member single Z1

				17	2	21	22	Member family Z1

				18	2	29	30	Member single Z2

				19	2	23	24	Member family Z2

				20	2	17	18	Member complimentary
				...
				case "1 Applied pending payment";

				case "2 Paid up";

				case "3 Renewal overdue";
				...
				case "1 Single within Europe"; 14

				case "2 Family within Europe"; 15

				case "3 Single outside Europe"; 16

				case "4 Family outside Europe"; 17
			...
			if (!$userid){
				...
				//update coupon table if new proposer
				if($coupon_email_recipient){
					...
					$insert->email_recipient = $coupon_email_recipient;

					$insert->user_id = $userid;

					$insert->date_proposed = $changedate;
...
<script src="https://ajax.googleapis.com/ajax/libs/jquery/2.1.1/jquery.min.js"></script>
		...
		function validateCoupon(box){

			var coupon = document.form.coupon.value;
			var myemail = document.form.Email.value;
			var couponvalidated = document.form.couponvalidated.value;
			var BasicSub = document.form.BasicSub.value;
						
			if(coupon&&myemail){
				//OK to validate	

				//test jim@newaddress.com
				
							
				
				$.ajax({
					url: '<?php echo($componentpath); ?>couponvalidate.php',
					type: 'POST',
					data: {coupon: coupon},
					success: function(result){
					  console.log(result);
					  if(result){ window.location.href="success.html"; }
					  else{ alert("Error: Failed to validate the referrer"); }
					}
				});

				document.form.couponvalidated.value='1';
				//change coupon entry to validated
				
				var span = document.getElementById("couponvalidate");
    			var el_down = document.getElementById("couponvalidate");
 
       			span.textContent = "&nbsp;referrer '"+coupon+"' validated - discount applied";
        		el_down.innerHTML = "&nbsp;referrer '"+coupon+"' validated - discount applied";
     
				changememtypeform();
			}else{
				alert("Please enter your email and the referrer's email address");
			}
			...
			var subamount=<?php if(!$BasicSub){echo(0);}else{echo($BasicSub);} ?>;
			...
			var ExistingMemTypeCode=<?php if($MemTypeCode){echo($MemTypeCode);}else{echo("0");} ?>;
...
?>				default:
			...
			if(document.form.service10.checked==true){
				...
				if(couponvalidated==1 && subamount>0){
					var coupon_subamount=subamount/2;
					document.form.BasicSub.value=coupon_subamount.toFixed(2);
					//alert("Your subscription has been reduced from "+subamount+" to "+coupon_subamount.toFixed(2));
				}else{
				...
				var coupon_subamount=subamount_d/2;
				if(couponvalidated==1 && subamount>0){
					document.form.BasicSub.value=coupon_subamount.toFixed(2);
					//alert("Your subscription has been reduced from "+subamount_d+" to "+coupon_subamount.toFixed(2));
				}else{
	  ...
	  <h2>My Details<?php
		...
		<td colspan="2"><b>Your  details</b> - the main subscriber and person responsible 
							...
							for upkeeping this profile. </td></tr>
	...
	<tr><td class=profile_label><?php echo($servicetitle); ?></td><td class=profile_field valign="top"><?php echo($boxes); ?><div id="discount_message"></div></td></tr>			



		<tr> 			  

		<td colspan="2"><font color=ff0000>Please use UPPER and lower case as these details will be used to address correspondence.</font></td>
		...
		<td class=profile_field> <input class=formcontrol type='text' name='Email' size='30' value="<?php echo($Email); ?>"></td>
			...
			<input type='hidden' name='couponvalidated' value='0'>
			
			<?php
		if($ID=="new"){

			echo("<tr><td class=profile_label>Member get Member referrer email</td><td class=profile_field><span id='couponvalidate'><input id='coupon' name='coupon' type='text' size='30' maxlength='50'><span class='btn btn-info' onclick='validateCoupon()'>Validate</span></span></td></tr>");
		...
		<td class=profile_field> <input class=formcontrol type='password' name='PW' size='30' value="<?php echo($PW); ?>">
			...
			<em>at least 8 characters</em> <span class="btn btn-info" onclick="generatePassword()">Generate Password</span></td>
		...
		<tr id="section2"><td colspan=2>Second family member</td></tr>
       ...
       <?php
		...
		<tr>
		  ...
		  <td colspan=2><strong>Postal address</strong> - <font color=ff0000>don't put the country as part of your address, it will be added from your 'Postal Country' above.</font></td>
...
<?php
		...
		Name: <?php if(!$ShipName){echo("<font color=ff0000><i> Please add this to the barge register</i></font>");}else{echo($ShipName);} ?><br>

		Class: <?php if(!$ShipClass){echo("<font color=ff0000><i> Please add this to the barge register</i></font>");}else{echo($ShipClass);} ?><br>	

		Year: <?php if($ShipYear<1){echo("<font color=ff0000><i> Please add this to the barge register</i></font>");}else{echo($ShipYear);} ?><br>	

		Length: <?php if($ShipLength<1){echo("<font color=ff0000><i> Please add this to the barge register</i></font>");}else{echo($ShipLength);} ?><br>

		Beam: <?php if($ShipBeam<1){echo("<font color=ff0000><i> Please add this to the barge register</i></font>");}else{echo($ShipBeam);} ?>	
		...
		<tr id="bargeowner2"><td class=profile_field>Cruising Country where you spend most of your time *</td>
		...
		<b>Pay by UK bank standing order.</b> If you have a UK bank account, select this option. Standing order means less work for our volunteers when renewing subscriptions each year but direct debit is even better.</td>
		...
		<b>Pay by secure credit or debit card.</b> For fast and secure payment by debit or credit card, select this option. An email reminder will be sent to you one month before renewal is due with instructions on how to pay securely on-line.</td>
		...
		<b>Pay by bank transfer or cheque.</b> If you have a UK bank account  select this option. An email reminder will be sent to you one month before renewal is due  with instructions on how to make a bank transfer or pay by cheque.</td>
		...
		<b>Pay by bank transfer.</b> If you have a UK bank account select this option. An email reminder will be sent to you one month before renewal is due with instructions on how to make a bank transfer.</td>
		...
		<b>Complimentary.</b> Select this option for Honarary or Press / Voucher members.</td>
	...
	if($membershipadmin==true && !$subaction){
		...
		and then Press 'Update >' where you will then have another chance to print letters which will include any updates.</td>
		...
		if($num_coupons>0){
			...
			echo("<td class=profile_label valign='top'></td><td class=profile_field valign='top'>Non made</td></tr>\n");
		}
		...
		<style type="text/css">@import url(../../popcal/calendar-blue.css);</style>
