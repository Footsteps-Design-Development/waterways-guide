...
/**
 * @version     1.0.0
 * @package     com_waterways_guide bargeregister
	...
	*/
		...
		if ($assetaction == "" || $assetaction == "list_1" || $assetaction == "list_2" || $assetaction == "list") {
			...
			foreach ($db->setQuery($query)->loadColumn() as $rowYearFrom) {
				...
				if ($option && $option == $assetyearfrom) {
					$selected = " selected";
				} else {
					$selected = "";
				}
			...
			while ($option < $LengthMax) {
				if ($option && $option == $assetlengthfrom) {
					$selected = " selected";
				} else {
					$selected = "";
				}
			...
			$option = 10;
			while ($option < $LengthMax) {
				if ($option && $option == $assetlengthto) {
					$selected = " selected";
				} else {
					$selected = "";
				}
			...
			$filter .= "<option selected value=\"*\">Any</option>\n";
			$option = "Unpowered";
			if ($option && $option == $assetpropulsion) {
				$selected = " selected";
			} else {
				$selected = "";
			}
		...
		?>
			<script language="JavaScript">
				...
				function MM_validateForm() {
					...
					if (errors) {
						alert('Please check and enter the following required information:\n' + errors);
					} else {
			...
			<tr>
				<td class='list_small_member'><?php echo ($contacthelp); ?></td>
			</tr>
			<tr>
				<td class='list_small'><a href="#" onClick="document.form.assetaction.value='detail';document.form.submit()">Back to the detail <img src="Image/common/back1.gif" width="18" height="18" border="0" alt="Back to the detail" title="Back to the detail"></a></td>
			</tr>
			<tr>
				<td class='list_small'>
						...
						<tr>
							<td colspan="2">Message<br>
								...
								<label>
									...
									Tick this box to receive an email copy of this message</label>
							</td>
						</tr>
					</table>
				</td>
